...
final_plot
ggsave("Figure 4.png", final_plot, width = 10, height = 10, dpi = 300, bg = "white")

filtered_data <- filtered_data %>%
  mutate(
    Group_Label = case_when(
      Population_Group == 3 ~ "20-30%",
      Population_Group == 4 ~ "30-40%",
      Population_Group == 10 ~ "90-100%",
      TRUE ~ NA_character_
    )
  )

comparison_data <- filtered_data %>%
  filter(Group_Label %in% c("20-30%", "30-40%", "90-100%"))

test_30_90 <- wilcox.test(total_accessibility ~ Group_Label,
                          data = filter(comparison_data, Group_Label %in% c("30-40%", "90-100%")))

test_20_90 <- wilcox.test(total_accessibility ~ Group_Label,
                          data = filter(comparison_data, Group_Label %in% c("20-30%", "90-100%")))

filtered_data <- merged_data %>%
  dplyr::select(district_name, `all population`, population_60, total_accessibility) %>%
  dplyr::mutate(
    all_population = `all population`,
    elderly_population = population_60,
    Population_Group = ntile(all_population, 10)
...
urban_districts <- c("东城区", "西城区", "朝阳区", "海淀区", "丰台区", "石景山区")

filtered_data <- filtered_data %>%
  mutate(
    region_type = ifelse(district_name %in% urban_districts, "Urban", "Suburban")
  )

region_count_by_group <- filtered_data %>%
  group_by(Population_Group, region_type) %>%
  summarise(count = n(), .groups = "drop") %>%
  pivot_wider(names_from = region_type, values_from = count, values_fill = 0) %>%
  mutate(
    Total = Urban + Suburban,
    Urban_Ratio = Urban / Total,
    Suburban_Ratio = Suburban / Total
  )

print(region_count_by_group)
