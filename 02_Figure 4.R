...
merged_data <- all_data %>%
  ...
  mutate(total_visits_1516 = ifelse(is.na(total_visits_1516), 0, total_visits_1516))  # 把 NA 赋值为 0
data <- merged_data %>%
...
p5_part <- ggplot(all_results, aes(x = time, y = mode_year, fill = year_label)) +
  geom_tile(width = 5, height = 0.8, color = "black", size = 0.3, alpha = 0.4) +  
  # geom_text(aes(label = coef_text), color = "black", fontface = "bold", size = 4) +  
  geom_text(aes(label = ifelse(p_value <= 0.10, coef_text, "")), color = "black", fontface = "bold", size = 5) +  
  ...
  labs(x = "Spatial accessibility (5-minute intervals)", y = "Mode of transport") +
  scale_y_discrete(
    ...
    labels = c("", "Driving", "", "", "Public Transport", "", "", "Walking", "", "", "Overall", "")
  ) +
  theme_minimal() +
  theme(
    ...
    axis.line = element_line(size = 1, color = "black"),      # x轴和y轴线加粗
    legend.position = "bottom",
    legend.title = element_text(size = 20, face = "bold"),
    legend.text = element_text(size = 20),
    strip.background = element_blank(),
    strip.text = element_blank(),
    plot.caption = element_text(hjust = 0.5, size = 18, face = "italic")
  ) +
  facet_wrap(~mode, scales = "free_y", ncol = 1, strip.position = "left") +
  labs(caption = "*** p ≤ 0.01, ** 0.01 < p ≤ 0.05, * 0.05 < p ≤ 0.10") +
  theme(plot.caption = element_text(hjust = 0.5, size = 16, face = "italic", lineheight = 1.4))

ggsave("Figure 4.png", p5_part, width = 16, height = 12, dpi = 300, bg = "white")  # 修改宽度、高度和分辨率
