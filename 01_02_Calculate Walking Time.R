...
coords_vaccination <- read_delim("D:/vaccination service accessibility/更新距离最近的三个疫苗接种点(周末).csv", delim = ";") %>%
  mutate(
...
for (i in 1:total_rows) {  
  ...
  tmp_url <- paste0(
    "https://restapi.amap.com/v3/direction/walking?origin=", origin[1], ",", origin[2],
  ...
  if (!is.null(response) && !is.null(response$route$paths)) {
    ...
    results_batch <- append(results_batch, list(pathdf)) 
  } else {
  ...
  if (i %% batch_size == 0 || i == total_rows) {  
    ...
    file_name <- paste0("walking_paths_results_weekend_morning_batch_", i_out %/% batch_size + 1, ".xlsx")
